...
**章节闭环**：
一个章节从上一刻系统能力开始，让问题真实发生，再依次提炼设计问题、引入最小概念、从零实现、迁移到 Mini DeerFlow，并用决策表、练习和验收收尾。
_Avoid_: Notebook 演示、代码片段集合

**概念实验室**：
核心概念第一次出现时使用的最小、透明、可运行实验。它先暴露真实失败，只增加一个新机制，并展示 State、事件、patch、合并或执行顺序；原则上不导入 Mini DeerFlow 封装。
_Avoid_: 玩具 Demo、封装调用示例、只含 assert 的单元测试

**工程迁移**：
概念实验完成后，把同一机制放回 Mini DeerFlow，解释完整项目新增的安全、持久化、抽象、复用和测试边界。它负责回答“放在哪里”，不负责概念的首次解释。
_Avoid_: 概念实验、源码导读、复制最小示例

**可观察执行记录**：
为学习者展示运行前状态、节点或工具输入、局部更新、合并过程、事件顺序和运行后状态的确定性输出。断言负责回归，执行记录负责建立心智模型。
_Avoid_: 只打印最终回答、只保留 assert、完整调试日志倾倒

**初学者验收 Agent**：
未参与课程设计和实现、只具备基础 Python 前提的独立评审者。它仅依赖正式 Web 章节和 Jupyter 练习建立概念，在课程指引前不读取项目源码或测试答案，并把无法继续、错误理解和被迫猜测记录为课程缺口。
_Avoid_: 熟悉项目的代码评审 Agent、替作者证明内容正确的测试 Agent、真实人类学习者的替代品
