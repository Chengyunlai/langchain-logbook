---
title: "第 01 章：先看见一次 Agent 运行"
description: "区分直接模型调用、Runnable 管道和 Agent 循环，并能正确读取 v2 流式事件。"
...
learningGoal: "区分直接模型调用、Runnable 管道和 Agent 循环，并能正确读取 v2 流式事件。"
contentType: "main"
---
...
消息、v1/v2 和各类 stream mode 的完整返回矩阵见[附录 A5](/langchain-logbook/posts/appendix/#a5-stream-mode-与-v1v2-返回形状)。
...
打开 [01_Getting_Started.ipynb](/langchain-logbook/notebooks/01_Getting_Started.ipynb)，依次运行真实模型最小调用、离线模型契约、Agent 工具循环和 v2 事件实验。
