...
这项校验只拒绝明显非法的数据，不等于完整 Sandbox。符号链接、路径解析、provider 生命周期和容器隔离将在 [Sandbox 与扩展专题](/langchain-logbook/posts/sandbox_extensions/)继续处理。
...
打开 [02_Structured_Output.ipynb](/langchain-logbook/notebooks/02_Structured_Output.ipynb)，依次运行真实模型抽取、离线结构化协议、计划、路径和失败结果实验。
