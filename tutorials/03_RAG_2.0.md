...
> [!NOTE]
> **本章只解决一个问题**：模型不知道最新或私有资料时，怎样先找证据再回答。
>
> **当前系统**：研究请求和计划已经是可验证对象。
>
> **遇到的问题**：结构正确不代表事实正确，模型仍可能使用过时知识并丢失来源。
>
> **本章目标**：建立 `Document → Chunk → Retriever → Prompt Context` 的带来源路径。
>
> **暂时不讲**：模型自主决定何时检索，以及完整 Agent 工具循环。
>
> **学完以后**：你能实现最小 RAG，并说明 Retriever 只返回 Document、不生成答案。
>
> **预计时间**：30～40 分钟。
...
> **确定性测试写法**：下面的 Fake Model 不会调用外部大模型，也不会读取证据并自行推理，只返回预设回答。它让我们稳定验证 Retriever、Context 和 source 的传递顺序；真实模型的回答质量要用 integration test 与评测另行检查。
