...
第 10 章让研究流程可以暂停、审批和恢复。现在，Lead Agent 既要查资料，又要检查 Python 接口，最后还要把两类结果综合成报告。

若每段网页、命令输出和工具轨迹都先回到 Lead，这些中间材料就会留在主消息历史。任务越长，最终综合看到的噪声越多，Secret 与内部记录也越容易被误传。

先不要急着创建“多个 Agent”。我们先确认普通函数为什么还不够，以及 specialist 应该看到和返回什么。
...
<!-- lesson-lab:id=ch11-context-pollution-failure layer=concept kind=failure concept=delegation-boundary pair=bounded-delegation -->
...
<!-- lesson-lab:id=ch11-bounded-delegation-repair layer=concept kind=repair concept=delegation-boundary pair=bounded-delegation -->
### 用请求与结果协议切断原始材料
...
<!-- lesson-lab:id=ch11-parent-context-leak-failure layer=concept kind=failure concept=context-projection pair=allowlist-projection -->
### 深拷贝仍然复制了完整主上下文
...
<!-- lesson-lab:id=ch11-context-projection-repair layer=concept kind=repair concept=context-projection pair=allowlist-projection -->
### 从空上下文开始，只投影允许字段
...
<!-- lesson-lab:id=ch11-command-router layer=concept kind=baseline concept=router-command -->
...
<!-- lesson-lab:id=ch11-send-router layer=concept kind=baseline concept=router-send -->
...
<!-- lesson-lab:id=ch11-handoff layer=concept kind=baseline concept=handoff-ownership -->
...
<!-- lesson-lab:id=ch11-subgraph-boundary layer=concept kind=contrast concept=subgraph-boundary -->
...
<!-- lesson-lab:id=ch11-ephemeral-specialist layer=concept kind=baseline concept=ephemeral-subagent -->
...
<!-- lesson-lab:id=ch11-unbounded-concurrency-failure layer=concept kind=failure concept=concurrency-control pair=executor-semaphore -->
...
<!-- lesson-lab:id=ch11-concurrency-limit layer=concept kind=repair concept=concurrency-control pair=executor-semaphore -->
### Semaphore 把峰值锁在执行入口
...
<!-- lesson-lab:id=ch11-gather-partial-failure layer=concept kind=failure concept=partial-failure pair=structured-batch-results -->
...
<!-- lesson-lab:id=ch11-timeout-partial-failure layer=concept kind=repair concept=partial-failure pair=structured-batch-results -->
...
<!-- lesson-lab:id=ch11-output-budget-failure layer=concept kind=failure concept=output-budget pair=bounded-result -->
...
<!-- lesson-lab:id=ch11-output-budget-repair layer=concept kind=repair concept=output-budget pair=bounded-result -->
...
<!-- lesson-lab:id=ch11-bounded-delegation-record layer=concept kind=baseline concept=delegation-ledger -->
### 只记录任务终态和边界事实
...
<!-- lesson-lab:id=ch11-isolated-specialists layer=migration kind=contrast concept=ephemeral-subagent -->
...
<!-- lesson-lab:id=ch11-task-tool layer=migration kind=contrast concept=delegation-boundary -->
...
<!-- lesson-lab:id=ch11-lead-agent-supervisor layer=migration kind=contrast concept=delegation-boundary -->
...
<!-- lesson-lab:id=ch11-mini-context-projection layer=migration kind=contrast concept=context-projection -->
...
<!-- lesson-lab:id=ch11-mini-concurrency-limit layer=migration kind=contrast concept=concurrency-control -->
...
<!-- lesson-lab:id=ch11-mini-partial-failure layer=migration kind=contrast concept=partial-failure -->
...
<!-- lesson-lab:id=ch11-mini-output-budget layer=migration kind=contrast concept=output-budget -->
...
<!-- lesson-lab:id=ch11-delegation-ledger-event layer=migration kind=contrast concept=delegation-ledger -->
...
## 8. 沿着 task 调用链读 DeerFlow

本章引用的 DeerFlow 源码固定在提交 `216309426fc6f954689ebee138af117029e43f8b`。不要从目录开始漫游，先按下面的 task 调用顺序阅读：
...
DeerFlow 的 specialist 不是固定流水线。Lead 在长期会话中动态决定是否调用 `task(description, prompt, subagent_type)`；每个 subagent 拥有自己的 Prompt、tools、skills、模型和预算。

Mini DeerFlow 没有复制 DeerFlow 的线程池、Gateway、SSE 和完整 runtime。它只保留读懂这条调用链所需的骨架：单一 task 接口、registry、隔离 invocation、受控并发、终态协议和有界 ledger。
...
## 11. Lead 最终只保留一条有界委派链

我们先让 Lead 的上下文被原始材料污染，再建立 TaskRequest、TaskResult、输入投影、控制权选择和执行预算。到这一步，Subagent 才不再只是“另一次模型调用”。

同一组机制随后进入 Mini DeerFlow：Registry 描述能力，Executor 守住运行边界，task tool 暴露单一接口，Lead 在 ToolMessage 返回后继续综合，Ledger 保存有界终态。

现在可以用控制权区分 Router、Handoff、Subgraph 与 Subagent-as-tool，用 `Command` 和 `Send` 表达路由与并行，并解释 Subgraph 为何不自动隔离上下文。并发、超时、部分失败和输出预算也都有了明确所有者。

下一篇进入[Mini DeerFlow 工程架构总览](../mini_deerflow/ARCHITECTURE.md)。不要只找类名，沿着已经建立的五条责任边界追踪：组合根、模型接口、执行接缝、持久化事实和外部能力。
...
继续阅读：[Mini DeerFlow 工程架构总览](../mini_deerflow/ARCHITECTURE.md)。
