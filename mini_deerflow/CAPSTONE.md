# Mini DeerFlow 最终综合实战：构建可恢复的研究交付 Agent
...
进入 DeerFlow 前先写下答案：哪个模块拥有 root graph？ThreadState 为什么不保存 Workspace 正文？task tool 如何裁剪上下文？RunJournal 与 trace provider 有何不同？Gateway 为什么还需要 Run/Event store？随后按 [`DEERFLOW_GUIDE.md`](./DEERFLOW_GUIDE.md) 沿调用链验证，而不是按目录漫游。
