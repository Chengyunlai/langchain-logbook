# Mini DeerFlow 综合实战：把一份研究报告可靠地交付出去
...
这里没有新的“Capstone 框架”。[第 03 章](../tutorials/03_RAG_2.0.md)留下本地检索，[Lead 专题](./LEAD_AGENT_CORE.md)留下 `create_agent` 组合根，[第 11 章](../tutorials/11_Multi_Agent_Patterns.md)留下委派接缝。

[Sandbox 专题](./SANDBOX_EXTENSIONS.md)负责草稿与正式产物，[第 06 章](../tutorials/06_Observability_Persistence.md)和[第 10 章](../tutorials/10_Human_In_The_Loop.md)负责跨进程审批，[评测专题](./EVALUATION_OBSERVABILITY.md)负责判断交付是否真的合格。
...
[Sandbox 专题](./SANDBOX_EXTENSIONS.md)已经给出了线程工作区。这里先把正文写入 `drafts/<request-id>.md`，checkpoint 只保存 path 和 SHA-256。四种事实因此各归其位：
...
真实写入完成后，最终评测再次检查完整轨迹。前一道门保护副作用，后一道评测保护回归基线；两者复用了[评测专题](./EVALUATION_OBSERVABILITY.md)的同一观察与 evaluator 接缝。
...
`capstone.py` 到这里已经证明 Harness 内的领域闭环。接入产品后，客户端不会直接等待这个 Python 函数返回；[Runtime 专题](./RUNTIME_GATEWAY.md)中的 `LocalRunManager` 或 Agent Server 要拥有 Run 生命周期：
...
随后按 [`DEERFLOW_GUIDE.md`](./DEERFLOW_GUIDE.md) 沿调用链逐一验证。若真实项目采用不同实现，先判断职责落在哪里，再比较目录名称。这样读到的是同一组工程问题在更大系统中的答案。

继续阅读：[从 Mini DeerFlow 进入真实 DeerFlow](./DEERFLOW_GUIDE.md)。
